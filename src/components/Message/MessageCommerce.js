...
class MessageCommerce extends PureComponent {
  ...
  render() {
    ...
    if (message.deleted_at) {
      return (
        <React.Fragment>
          <span key={message.id} className={`${messageClasses} str-chat__message--deleted`}>
      ...
      <React.Fragment>
        <div
          ...
          {(groupStyles[0] === 'bottom' || groupStyles[0] === 'single') && (
            <Avatar image={message.user.image} size={32} name={message.user.name || message.user.id} />
          )}
          <div className="str-chat__message-commerce-inner">
              ...
              <div className="str-chat__message-commerce-text">
                <div
                  className={`str-chat__message-commerce-text-inner
									...
									${isOnlyEmojis(message.text) ? 'str-chat__message-commerce-text-inner--is-emoji' : ''}
                `.trim()}
                  onMouseOver={onMentionsHoverMessage}
                  onClick={onMentionsClickMessage}>
                  {message.type === 'error' && (
                    <div className="str-chat__commerce-message--error-message">{t('Error · Unsent')}</div>
                  )}
            ...
            <div className={`str-chat__message-commerce-data`}>
              {this.isMine() ? (
                <span className="str-chat__message-commerce-name">{message.user.name || message.user.id}</span>
              ) : null}
