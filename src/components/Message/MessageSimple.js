...
class MessageSimple extends PureComponent {
  ...
  renderStatus = () => {
    const { readBy, client, message, threadList, lastReceivedId, t } = this.props;
    ...
    if (message.status === 'sending') {
      ...
      return (
        <span className="str-chat__message-simple-status">
          ...
          {readBy.length > 2 && <span className="str-chat__message-simple-status-number">{readBy.length - 1}</span>}
        </span>
      );
  ...
  renderOptions() {
    const { message, initialMessage, channelConfig, threadList, handleOpenThread } = this.props;
    if (
      message.type === 'error' ||
      message.type === 'system' ||
  ...
  render() {
    ...
    const images = hasAttachment && message.attachments.filter((item) => item.type === 'image');
    const hasReactions = Boolean(message.latest_reactions && message.latest_reactions.length);
    ...
    if (message.deleted_at) {
      return (
        <React.Fragment>
          <div key={message.id} className={`${messageClasses} str-chat__message--deleted ${message.type} `}>
            ...
            {message.text && (
              <div className="str-chat__message-text">
                ...
                `.trim()}
                  ...
                  {message.type === 'error' && (
                    <div className="str-chat__simple-message--error-message">{t('Error · Unsent')}</div>
                  )}
                  {message.status === 'failed' && (
